...
public class MyHouseActivity extends Activity implements OnItemClickListener,
	...
	private void initDate() {
		 ...
		 * HouseItemBean(); houseBean.setHouse_item_title("[单间] [整租] [家电齐全]");
	...
	private void getMoreDate() {
		 ...
		 * HouseItemBean(); houseBean.setHouse_item_title("更多加载");
		 * houseBean.setHouse_item_add("鲁巷-吴家湾大厦租房");
		 * houseBean.setHouse_item_rental("1200"); list.add(houseBean); }
		 */
	}
	...
	private class MyAsyncTask extends AsyncTask<String, Integer, String> {
		...
		protected void onPreExecute() {
			...
			progressDialog = ProgressDialog.show(MyHouseActivity.this, "","正在获取信息，请稍等。。。");
		}
		...
		protected void onPostExecute(String result) {
			...
			if ("serverweb is error".equals(result)) {
				Toast.makeText(MyHouseActivity.this, "服务器异常！请稍后重试。。。",Toast.LENGTH_SHORT).show();
			} else if ("webserver is stop".equals(result)) {
				Toast.makeText(MyHouseActivity.this, "服务器停止运行！请稍后重试。。。",Toast.LENGTH_SHORT).show();
			} else {
				...
				if ("Error".equals(resultState)) {
					Toast.makeText(MyHouseActivity.this, "服务器错误，请稍后再试！",
					...
					try {
						...
						if (houseList != null) {
							for (int i = 0; i < houseList.length(); i++) {
								...
								list.add(houseBean);
							Log.d("房源列表解析到的数据：", houseList.getJSONObject(i).getString("house_item_img"));
							}
							//如果加载到数据，则通知适配器更新界面
							showListView();
							houseList=null;
							...
							// 否则提示用户数据加载完毕
							Toast.makeText(MyHouseActivity.this,"数据已全部加载！", Toast.LENGTH_SHORT).show();
						}
					} catch (JSONException e) {
						Log.d("显示房源列表", "数据解析异常");
					}
